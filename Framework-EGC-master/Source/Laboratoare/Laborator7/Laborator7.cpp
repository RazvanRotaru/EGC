...
void Laborator7::Init()
{
	...
	//Light & material properties
	{
		...
		materialKd = glm::vec3(0.5);
		materialKs = glm::vec3(0.5);
	}
}
...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab7"], modelMatrix, materialKd, materialKs, 30);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0.2, 0.7, 0.1), materialKs, 28);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0, 0.5, 0), materialKs, 20);
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0, 0.5, 0), materialKs, 24);
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &Kd, const glm::vec3 &Ks, const float shininess)
{

	//glm::vec3 world_pos = modelMatrix* glm::vec4()
	...
	GLint loc_LP = glGetUniformLocation(shader->program, "light_position");
	glUniform3f(loc_LP, lightPosition.x, lightPosition.y, lightPosition.z);

	// TODO: Set eye position (camera position) uniform
	glm::vec3 eyePosition = GetSceneCamera()->transform->GetWorldPosition();
	GLint loc_EP = glGetUniformLocation(shader->program, "eye_position");
	glUniform3fv(loc_EP, 1, glm::value_ptr(eyePosition));

	// TODO: Set material property uniforms (shininess, kd, ks, object color) 
	GLint loc_KD = glGetUniformLocation(shader->program, "material_kd");
	glUniform3fv(loc_KD, 1, glm::value_ptr(Kd));

	GLint loc_KS = glGetUniformLocation(shader->program, "material_ks");
	glUniform3fv(loc_KS, 1, glm::value_ptr(Ks));

	GLint loc_SH = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(loc_SH, shininess);
