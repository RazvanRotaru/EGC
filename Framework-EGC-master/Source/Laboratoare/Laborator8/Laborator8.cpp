...
void Laborator8::Update(float deltaTimeSeconds) {
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0, 0.5));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color) {
	...
	int spotLightLoc = glGetUniformLocation(shader->program, "spotLight");
...
void Laborator8::OnInputUpdate(float deltaTime, int mods) {
	...
	if (window->KeyHold(GLFW_KEY_UP)) {
		...
		m = Transform3D::RotateOX(deltaTime * speed);
		lightDirection = glm::vec3(m * glm::vec4(lightDirection, 1));
	}
	...
	if (window->KeyHold(GLFW_KEY_LEFT)) {
		glm::mat4 m = glm::mat4(1);
		m = Transform3D::RotateOZ(-deltaTime * speed);
		lightDirection = glm::vec3(m * glm::vec4(lightDirection, 1));
	}
	...
	if (window->KeyHold(GLFW_KEY_N)) cutOff += deltaTime;
	if (window->KeyHold(GLFW_KEY_M)) cutOff -= deltaTime;
}
