...
void Laborator9::Update(float deltaTimeSeconds) {
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"], 1);
	}
...

	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(30), GetSceneCamera()->transform->GetLocalOYVector());
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], 2);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, int varryOY, Texture2D* texture2) {
	...
	if (varryOY == 1) {
		...
		glUniform1i(glGetUniformLocation(shader->program, "useTime"), 1);
	} else {
		glUniform1i(glGetUniformLocation(shader->program, "useTime"), 0);
	}
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height) {
	...
	GLenum wrappingMode = GL_CLAMP_TO_EDGE;
...
void Laborator9::OnInputUpdate(float deltaTime, int mods) {
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) {
		...
		forward = GetSceneCamera()->transform->GetLocalOZVector();
